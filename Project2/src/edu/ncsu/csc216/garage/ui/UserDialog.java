...
public class UserDialog extends JDialog {
	 ...
	 */
	private static final long serialVersionUID = -3044035408941234284L;
	/** Border text */
	 ...
	 * Constructor. Sets up the dialog window
	 * 
	 * @param t the Tiered object to begin editing
	 */
	...
	/**
	 * Private method. Creates the grid to lay out the labels and widgets with
	 * values for the selected Vehicle pre-filled. Only the Tier field is enabled.
	 * 
	 * @param t
	 */
	private void setInfoForEdit(Tiered t) {
		boolean hev = false;
		Vehicle v = (Vehicle) t;
		if (v instanceof HybridElectricCar) {
			hev = true;
			System.out.println("hev" + hev);
		}
		ckbxKind.setSelected(hev);
		ckbxKind.setEnabled(false);
		JTextField oField = (JTextField) compInfo[0];
		oField.setText(v.getName());
		oField.setEnabled(false);
		JTextField lField = (JTextField) compInfo[1];
		lField.setText(v.getLicense());
	 ...
	 * @throws BadVehicleInformationException if a Car cannot be created from the
	 *                                        given information
	 */
